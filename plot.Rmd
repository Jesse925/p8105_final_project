...
voline_plot = drug_df %>%
         mutate(drug_name = case_when(drug_name == "any_opioid"~"opioid",
                                      drug_name == "opiate_nos" ~ "opioid",
                                      drug_name == "fentanyl_analogue" ~ "fentanyl",
                                      TRUE ~ as.character(drug_name)),
                
                type = case_when(drug_name == "heroin" ~ "Natural drugs",
         ...
         group_by(drug) %>%
         mutate(count = n()) %>%
         select(age, race, sex, drug, count,type) %>%
         drop_na() %>%
         arrange(desc(count))
...
voline_plot %>%
  filter(drug == "fentanyl"|drug =="opioid"|drug =="heroin")%>%
  arrange(count)%>%
  plot_ly(type = 'violin',orientation ="h") %>%
  add_trace(
    y = ~drug[voline_plot$sex == 'Male'], x = ~age[voline_plot$sex == 'Male'],
    ...
    line = list(color = '')) %>%
  add_trace(
    y = ~drug[voline_plot$sex == 'Female'], x = ~age[voline_plot$sex == 'Female'],
...
voline_plot %>%
  plot_ly(type = 'violin') %>%
  add_trace(
    x = ~type[voline_plot$sex == 'Male'], y = ~age[voline_plot$sex == 'Male'],
    legendgroup = 'Male', scalegroup = 'Male', name = 'Male',
    side = 'negative',
    box = list(visible = T),
    meanline = list(visible = T),
    line = list(color = '')) %>%
  add_trace(
    x = ~type[voline_plot$sex == 'Female'], y = ~age[voline_plot$sex == 'Female'],
    legendgroup = 'Female', scalegroup = 'Female', name = 'Female',
    side = 'positive',
    box = list(visible = T),
    meanline = list(visible = T),
